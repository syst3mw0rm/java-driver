...
public class TableMetadata {
    ...
    static TableMetadata build(KeyspaceMetadata ksm, Row row, Map<String, ColumnMetadata.Raw> rawCols, VersionNumber cassandraVersion, String nameColumn) {
        ...
        if(cassandraVersion.getMajor() > 2) {
            ...
            if(isCompact) {
                rawCols = maybePruneAutoGeneratedColumns(rawCols);
            }
    ...
    private static Map<String, ColumnMetadata.Raw> maybePruneAutoGeneratedColumns(Map<String, ColumnMetadata.Raw> rawCols) {
        // Upon migration from thrift to CQL, we internally create a pair of surrogate clustering/regular columns
        // for compact static tables. These shouldn't be exposed to the user but are currently returned by C*.
        // Heuristic: if a COMPACT STORAGE table contains static columns, it means some columns were auto-generated
        Collection<ColumnMetadata.Raw> cols = rawCols.values();
        if(hasStaticColumns(cols)) {
    ...
    private static boolean hasStaticColumns(Collection<ColumnMetadata.Raw> cols) {
        return Iterables.any(cols, new Predicate<ColumnMetadata.Raw>() {
            @Override
            public boolean apply(ColumnMetadata.Raw input) {
                return input.kind == ColumnMetadata.Raw.Kind.STATIC;
            }
        });
    }
