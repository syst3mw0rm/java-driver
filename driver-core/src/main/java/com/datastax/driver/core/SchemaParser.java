...
abstract class SchemaParser {
    ...
    private static SchemaParser V2_PARSER = new SchemaParser() {
        ...
        void refresh(Metadata metadata,
            ...
            Map<String, Map<String, Map<String, ColumnMetadata.Raw>>> colsDefs = groupByKeyspaceAndCf(get(colsFuture), cassandraVersion, CF_NAME);
    ...
    private static SchemaParser V3_PARSER = new SchemaParser() {
        ...
        void refresh(Metadata metadata,
            ...
            Map<String, List<Row>> udtDefs = groupByKeyspace(get(udtFuture));
            Map<String, List<Row>> cfDefs = groupByKeyspace(get(cfFuture));
            Map<String, Map<String, Map<String, ColumnMetadata.Raw>>> colsDefs = groupByKeyspaceAndCf(get(colsFuture), cassandraVersion, TABLE_NAME);
            Map<String, List<Row>> functionDefs = groupByKeyspace(get(functionsFuture));
            Map<String, List<Row>> aggregateDefs = groupByKeyspace(get(aggregatesFuture));
    ...
    static void buildTableMetadata(KeyspaceMetadata ksm, List<Row> cfRows, Map<String, Map<String, ColumnMetadata.Raw>> colsDefs, VersionNumber cassandraVersion, String tableName) {
        for (Row cfRow : cfRows) {
            ...
            try {
                ...
                ksm.add(TableMetadata.build(ksm, cfRow, cols, cassandraVersion, tableName));
    ...
    static void buildUserTypes(KeyspaceMetadata ksm, List<Row> udtRows) {
        for (Row row : udtRows)
            ksm.add(UserType.build(row));
    }

    static void buildFunctionMetadata(KeyspaceMetadata ksm, List<Row> rows) {
        for (Row row : rows)
            ksm.add(FunctionMetadata.build(ksm, row));
    }

    static void buildAggregateMetadata(KeyspaceMetadata ksm, List<Row> rows, ProtocolVersion protocolVersion) {
        for (Row row : rows)
            ksm.add(AggregateMetadata.build(ksm, row, protocolVersion));
    }
